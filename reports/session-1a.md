...
| SC-3 | Inserting an `approval` with `decided_by = requested_by` is rejected | pass | `scaffold/tests/approval.test.ts::SC-3 an approval decided by its own requester is rejected`, `::SC-3 the same request decided by a second actor is accepted` | `npm test` → `✓ scaffold/tests/approval.test.ts (6 tests)`; rejection carries `constraint: approval_maker_checker` |
| SC-4 | `requested_by` ≠ `app.actor_id` on insert, or `decided_by` ≠ `app.actor_id` on update, is rejected | pass | `scaffold/tests/approval.test.ts::SC-4 requested_by that is not the transaction actor is rejected`, `::SC-4 decided_by that is not the transaction actor is rejected` | `npm test`; errors: `approval.requested_by (…) must equal app.actor_id (…)` and `approval.decided_by (…) must equal app.actor_id (…)` |
